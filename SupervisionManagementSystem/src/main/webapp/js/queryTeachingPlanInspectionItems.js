...
function submitAdvancedSearchData(){
	...
	submitDataByForm("queryTeachingPlanInspectionItems",{id:id});
...
function getUpdateAreaDataFromTable(updateImage) {
	var id,content,optionsArray;
	
	var tdList=updateImage.parents("tr").children();
	
	id=tdList.eq(1).text();
	content=tdList.eq(2).text();
	optionsArray=tdList.eq(3).text().replace(/▢/g,"").split(" ");
	...
	for(var i=0;i<optionsArray.length;i++){
		...
		uOptionsInput.val(options);
		if(i>=count){
			var html="<tr><td>选项"+(++count)+"：</td><td><input type='text' name='options' value="+options+"></td><td class='errors'></td></tr>";
	...
	optionsErrorTdList=$(".errors").filter(":not(:eq(0)):not(:eq(0))");
}
...
function submitUpdateAreaData(){
	...
	for(var i=0;i<uOptionsInputList.length;i++){
		var uOptionsInput=uOptionsInputList.eq(i);
		if(i==0){
			options="▢"+uOptionsInput.val();
			continue;
		}
		options+=" ▢"+uOptionsInput.val();
	}
	...
	$.ajax({
        type: "POST",
        url: "updateTeachingPlanInspectionItems",
...
$(document).ready(function(){
	...
	optionsErrorTdList=$(".errors:not(:eq(0)):not(:eq(0))");
	
	errorMessageP=updateDataArea.find(".errorMessage");
	optionsTd=updateDataTab.find("td:contains(选项)").eq(0);
	...
	batchDeleteBtn.click(function(){
		...
		submitDataByForm("deleteTeachingPlanInspectionItems",{idList:idList});
	});
	...
	updateImageList.each(function(index){
	...
	deleteImageList.each(function(index){
		var deleteImage=deleteImageList.eq(index);
		deleteImage.click(function(){
			if(confirm("确认删除该数据？")) { 
				submitDataByForm("deleteTeachingPlanInspectionItems",{idList:[deleteImage.parents("tr").children(":nth-child(2)").text()]});
			}
		});
	});
