...
function submitAdvancedSearchData(){
	...
	submitDataByForm("queryDailyInspectionItems",{id:id});
...
function submitUpdateAreaData(){
	...
	$.ajax({
        type: "POST",
        url: "updateDailyInspectionItems",
        dataType: "json",
...
$(document).ready(function(){
	...
	batchDeleteBtn.click(function(){
		...
		submitDataByForm("deleteDailyInspectionItems",{idList:idList});
	});
	...
	deleteImageList.each(function(index,ele){
		...
		deleteImage.click(function(){
			if(confirm("确认删除该数据？")) { 
				submitDataByForm("deleteDailyInspectionItems",{idList:[deleteImage.parents("tr").children(":nth-child(2)").text()]});
			}
		});
	});
