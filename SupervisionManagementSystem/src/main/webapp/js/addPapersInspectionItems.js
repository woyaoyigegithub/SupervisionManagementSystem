/**
 * 1. 所有发送表单请求均规定为insertXXX,queryXXX,updateXXX,deleteXXX.（例：insertPapersInspectionItems）
...
function verifyData(){
	...
	optionsInputList.each(function(index){
		...
		//字词判断
		else if(!vOptions.test(optionsInput.val())){
...
//获取表单数据，发起（url：addPapersInspectionItems）http请求到后台。
function submitData(){
	var content,options;
	...
	optionsInputList.each(function(index){
		var optionsInput=optionsInputList.eq(index);
		if(index==0){ options="▢"+optionsInput.val(); }
		else{ options+=" ▢"+optionsInput.val(); }
	});
	
	$.ajax({
        type: "POST",
        url: "insertPapersInspectionItems",
        dataType: "json",
        data:{
        	content:content,
        	options:options
...
$(document).ready(function(){
	...
	contentTextarea=dataTab.find("textarea[name='content']");
	optionsInputList=dataTab.find("input[name='options']");
	...
	optionsTd=dataTab.find("td:contains(抽查情况选项)");
